...
- **Unrelated finding on the same project**: the advisors run above also flagged `public.movies` (RLS disabled, exposed to the anon key) and a `SECURITY DEFINER` view (`public.daily_focus_logs`). Neither is part of this app's schema (`001_initial_schema.sql` only creates `profiles`/`tasks`/`focus_sessions`/`user_settings`) — they look like leftover data from something else in the same Supabase project. Worth a look since it shares this app's anon key, but out of scope for Flowmodora's own code.
