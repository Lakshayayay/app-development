...
The UI may refresh every second, but refresh cadence is never the source of truth. On relaunch the engine decodes `TimerSnapshot`, and the first refresh immediately catches up any countdown that ended while the app was not visible. A completed focus session is written once using its UUID; recording is idempotent if a completion event is repeated.
